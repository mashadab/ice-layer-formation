...
#parameters
##simulation
simulation_name = f'figure3_analytical_4Cycles_-20WCold_sine'
diffusion = 'yes'
CFL    = 0.1     #CFL number
tilt_angle = 0   #angle of the slope
ncycles = 3           
npp    = 3   #number of positive days [days]          
break_type = 'equal' #equal or unequal
break_length = 5    #number of days for a break         
lag    = npp #number of days to wait after preipitation has happened in the very end[days]
pulse_type = 'sine'  #'sine' wave or 'square' wave  : sine needs equal break

On_flux  =  20  #Flux on hot days [W/m^2]
...
H       = enthalpyfromT(T_analytical_sine(Yc_col,0)+Tm,Tm,rho_i,rho_w,0,cp_i,cp_w,0,phi_w,phi_nw,L_fusion) #Constant
...
'''
#for loading data
data = np.load('Hyperbolic-thermo-DYE2-actual-flux-lateral_C0.03_200by200_t10368000.npz')
t=data['t']
...
line3, = ax3.plot(y, x, lw=2, color= 'b',linestyle='--')
line4, = ax3.plot(z, x, lw=2, color= 'r')
line5, = ax3.plot(z, x, lw=2, color= 'k',linestyle='--')
ax2.set_title("Time: %0.2f days" %t[0],loc = 'center', fontsize=18)
plt.tight_layout()
...
def animate(i,u_sol,Sw_sol,phi_w_sol,phi_i_sol,Yc,Yf,time):
    ...
    z = np.transpose(phi_w_sol[:,i].reshape(Grid.Nx,Grid.Ny))[:,1] 
    zz= 1-np.transpose(phi_i_sol[:,i].reshape(Grid.Nx,Grid.Ny))[:,1] 
    T_analy = T_analytical_sine(x,time[i]*day2s)                        
    Tmelt = 0.0*Yc[:,1]
    line[0].set_data( zz, x)
    line[1].set_data(z, x)
    line[2].set_data(Tmelt, x)
    line[3].set_data(y, x)
    line[4].set_data(T_analy, x)
    #plt.tight_layout()
...
ani.save(f"../Figures/{simulation_name}_combined_with_seconds_analytical.mov", writer='ffmpeg', fps=30)
...
plt.plot(1-phi_i_sol[0:Grid.Ny,np.argwhere(t/day2s == 2*npp)[0][0]],Grid.yc,'r',label=f'{2*npp} days')
plt.plot(1-phi_i_sol[0:Grid.Ny,np.argwhere(t/day2s == 4*npp)[0][0]],Grid.yc,'g',label=f'{4*npp} days')
plt.plot(1-phi_i_sol[0:Grid.Ny,np.argwhere(t/day2s == 6*npp)[0][0]],Grid.yc,'b',label=f'{6*npp} days')
plt.plot(1-phi_i_sol[0:Grid.Ny,np.argwhere(t/day2s == 8*npp)[0][0]],Grid.yc,'k',label=f'{8*npp} days')
plt.plot(1-phi_i_sol[0:Grid.Ny,np.argwhere(t == tf)[0][0]],Grid.yc,'r--',label=f'{int(tf/day2s)} days')
plt.ylabel(r'$z$ [m]')
plt.xlabel(r'$\phi$')
