...
#parameters
##simulation
simulation_name = f'asym_figure_5Cycles_32.5Wto-32.5Wminus20Cold_sine'
diffusion = 'yes'
CFL    = 0.1     #CFL number
...
lag    = 0#2*npp#npp #number of days to wait after preipitation has happened in the very end[days]
...
while time<tmax:
    if break_type == 'equal':
        if pulse_type =='sine':
            ...
            #Param.H.qb = mean_flux + dflux * np.sin(2*np.pi*time/(day2s*npp*2) + np.arcsin(-mean_flux/dflux))* np.ones((len(dof_inj),1))
...
#Remove 99% melted ice
phi_sol = 1-phi_i_sol
phi_i_sol_copy = phi_i_sol.copy()
phi_w_sol_backup[phi_sol>1-non_porous_vol_frac] = np.nan
phi_w_sol[phi_sol>1-non_porous_vol_frac] = np.nan
phi_i_sol_copy[phi_sol>1- non_porous_vol_frac] = np.nan
T_sol_copy = T_sol.copy()
T_sol_copy[phi_sol>1-non_porous_vol_frac] = np.nan
T_sol_copy[np.isnan(phi_i_sol_copy)] = np.nan
phi_w_sol_backup[np.isnan(phi_i_sol_copy)] = np.nan
phi_w_sol[np.isnan(phi_i_sol_copy)] = np.nan
phi_i_sol_copy[phi_sol>1- non_porous_vol_frac] = np.nan
#####
...
t_array=np.kron(tday,np.ones((Grid.Ny,1)))
phi_w_array = phi_w_sol_backup[int(Grid.Nx*Grid.Ny/2+0):int(Grid.Nx*Grid.Ny/2+Grid.Ny),:]
phi_i_array = phi_i_sol_copy[int(Grid.Nx*Grid.Ny/2+0):int(Grid.Nx*Grid.Ny/2+Grid.Ny),:]
...
plot = [plt.contourf(t_array, depth_array, (1-phi_i_array),cmap="Greys",levels=100,ls=None,vmin=0.0,vmax=1)]
mm = plt.cm.ScalarMappable(cmap=cm.Greys)
mm.set_array(np.linspace(0.0,1,10000))
#mm.set_array(1-phi_i_sol_copy)
plt.yticks([0,1,2,3])
plt.ylabel(r'Depth [m]')
plt.xlim([tday[0],tday[-1]])
plt.ylim([Grid.ymax,Grid.ymin])
...
T_array = T_sol_copy[int(Grid.Nx*Grid.Ny/2+0):int(Grid.Nx*Grid.Ny/2+Grid.Ny),:]
...
[N,frn] = np.shape(T_sol_copy) # frame number of the animation from the saved file
[Xy,Yf] = np.meshgrid(Grid.xf,Grid.yf)
# First set up the figure, the axis, and the plot element we want to animate
fig = plt.figure(figsize=(15,7.5) , dpi=100)
ax1 = fig.add_subplot(1, 3, 1)
...
ax3.set_xlim([-30,0])#np.max(T_sol_copy)-273.16])
ax3.set_ylim([ymax_loc,Grid.ymin])
ax3.set_xlabel(r'T[$^\circ$C]')
...
#ax2.set_xlim([np.min(phi_w_sol/(1-phi_i_sol_copy)),np.max(phi_w_sol/(1-phi_i_sol_copy))])
ax2.set_xlim([0,0.1])
ax2.set_ylim([ymax_loc,Grid.ymin])
ax2.set_xlabel(r'$LWC$')
...
# animation function.  This is called sequentially
def animate(i,u_sol,Sw_sol,phi_w_sol,phi_i_sol_copy,Yc,Yf,time):
    ax1.set_xlim([0,1])
    x = Yc[:,1] 
    y = np.transpose(u_sol[:,i].reshape(Grid.Nx,Grid.Ny))[:,1]-273.16    
    z = np.transpose(phi_w_sol[:,i].reshape(Grid.Nx,Grid.Ny))[:,1] 
    zz= 1-np.transpose(phi_i_sol_copy[:,i].reshape(Grid.Nx,Grid.Ny))[:,1] 
    zz0= 1-np.transpose(phi_i_sol_copy[:,0].reshape(Grid.Nx,Grid.Ny))[:,1]                           
    Tmelt = 0.0*Yc[:,1]
    line[0].set_data( zz, x)
    line[1].set_data(z, x)
    line[2].set_data(Tmelt, x)
...
# call the animator.  blit=True means only re-draw the parts that have changed.
ii=1
ani = animation.FuncAnimation(fig, animate, frn, init_func=init, fargs=(T_sol_copy[:,::ii],s_w_sol[:,::ii],phi_w_sol[:,::ii],phi_i_sol_copy[:,::ii],Yc[:,:],Yf[:,:],tday[::ii])
                               , interval=1/fps)
...
Grid.ymax = 1.25#5#1.25#5#3.5#1#3.0#4.5
fig = plt.figure(figsize=(5,15) , dpi=100)
plt.plot(1-phi_i_sol_copy[0:Grid.Ny,np.argwhere(t/day2s == 0)[0][0]],Grid.yc,'k-',label=f'{0*npp} days',color = black_array[5,:])
plt.plot(1-phi_i_sol_copy[0:Grid.Ny,np.argwhere(t/day2s == 2*npp)[0][0]],Grid.yc,'k-',label=f'{2*npp} days',color = black_array[4,:])
plt.plot(1-phi_i_sol_copy[0:Grid.Ny,np.argwhere(t/day2s == 4*npp)[0][0]],Grid.yc,'k-',label=f'{4*npp} days',color = black_array[3,:])
plt.plot(1-phi_i_sol_copy[0:Grid.Ny,np.argwhere(t/day2s == 6*npp)[0][0]],Grid.yc,'k-',label=f'{6*npp} days',color = black_array[2,:])
plt.plot(1-phi_i_sol_copy[0:Grid.Ny,np.argwhere(t/day2s == 8*npp)[0][0]],Grid.yc,'k-',label=f'{8*npp} days',color = black_array[1,:])
plt.plot(1-phi_i_sol_copy[0:Grid.Ny,np.argwhere(t == tf)[0][0]],Grid.yc,'k-',label=f'{10*npp} days',color = black_array[0,:])
plt.ylabel(r'$z$ [m]')
plt.xlabel(r'$\phi$')
